...
public sealed class AlbumService : IAlbumService
{
    ...
    /// <inheritdoc/>
    public async Task<IReadOnlyList<AlbumWithPhotos>> GetAlbumsWithPhotosAsync(CancellationToken cancellationToken = default)
    {
        ...
        var photosByAlbumId = photoDtos
            ...
            .ToDictionary(
                g => g.Key,
                g => g.Select(MapPhoto).ToList());

        return albumDtos.Select(albumDto =>
        {
    ...
    /// <inheritdoc/>
    public async Task<IReadOnlyList<AlbumWithPhotos>> GetAlbumsWithPhotosByUserAsync(int userId, CancellationToken cancellationToken = default)
    {
    ...
    private async Task<(IReadOnlyList<AlbumDto> Albums, IReadOnlyList<PhotoDto> Photos)> FetchBothAsync(CancellationToken cancellationToken)
    {
