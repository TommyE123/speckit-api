...
public static class AlbumsEndpoints
{
            ...
            HttpContext httpContext) =>
        {
            if (userId is not null)
            {
                if (!int.TryParse(userId, out var parsedId) || parsedId <= 0)
                {
                    ...
                    return Results.Json(
                        new ErrorResponse("userId must be a positive integer.", "INVALID_PARAMETER", correlationId),
                        statusCode: 400);
                }
    ...
    private static IReadOnlyList<AlbumWithPhotosResponse> MapToResponse(IReadOnlyList<Models.AlbumWithPhotos> albums) =>
        albums.Select(a => new AlbumWithPhotosResponse(
            new AlbumResponse(a.Album.Id, a.Album.UserId, a.Album.Title),
            a.Photos.Select(p => new PhotoResponse(p.Id, p.AlbumId, p.Title, p.ImageUrl, p.ThumbnailUrl)).ToList()
        )).ToList();
}
